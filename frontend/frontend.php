...
class VI_WP_LUCKY_WHEEL_Frontend_Frontend {
    ...
    public function send_email( $user_email, $customer_name, $mobile = '', $value = '', $label = '', $language = '' ) {
      ...
      $admin_email_temp = $this->settings->get_params( 'result', 'admin_email', $language );
        $admin_email_to   = $admin_email_temp['to'];
        $admin_email_enabled = ($admin_email_temp['enable'] == 'on');
        $admin_content    = stripslashes( nl2br( $admin_email_temp['content'] ) );
        $admin_content    = str_replace( '{prize_label}', $label, $admin_content );
        $admin_content    = str_replace( '{customer_name}', $customer_name, $admin_content );
        $admin_content    = str_replace( '{customer_email}', $user_email, $admin_content );
        $admin_content    = str_replace( '{customer_mobile}', $mobile, $admin_content );
        $admin_content    = str_replace( '{prize_value}', $value, $admin_content );
        $admin_content    = str_replace( '{today}', $now, $admin_content );
        $admin_subject    = stripslashes( $admin_email_temp['subject'] );
        $admin_subject    = stripslashes( nl2br($admin_email_temp['subject'] ));
        $admin_subject    = str_replace( '{prize_label}', $label, $admin_subject );
        $admin_subject    = str_replace( '{customer_name}', $customer_name, $admin_subject );
        $admin_subject    = str_replace( '{customer_email}', $user_email, $admin_subject );
        $admin_subject    = str_replace( '{customer_mobile}', $mobile, $admin_subject );
        $admin_subject    = str_replace( '{prize_value}', $value, $admin_subject );
        $admin_subject    = str_replace( '{today}', $now, $admin_subject );

        
        $admin_email_heading = isset( $admin_email_temp['heading'] ) ? $admin_email_temp['heading'] : '';
    ...
    display: block;">
                                                <h1><?php echo $admin_email_heading; ?></h1>
                                            </td>
                                        </tr>
                                    </table>
                                    <!-- End Header -->
        ...
        <?php
            ...
            wp_mail( $admin_email_to, $admin_subject, $admin_content, $header );
        }
